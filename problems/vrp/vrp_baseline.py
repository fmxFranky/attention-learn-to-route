...
def get_lkh_executable(
    ...
    if not os.path.isdir(filedir):
        ...
        assert os.path.isfile(
            file), "Download failed, {} does not exist".format(file)
        check_call(["tar", "xvfz", file], cwd=cwd)

        assert os.path.isdir(
            filedir), "Extracting failed, dir {} does not exist".format(
                filedir)
...
def write_vrplib(filename,
    ...
    with open(filename, 'w') as f:
        f.write("\n".join([
            "{} : {}".format(k, v)
            for k, v in (("NAME", name), ("TYPE", "CVRP"), ("DIMENSION",
                                                            len(loc) + 1),
                         ("EDGE_WEIGHT_TYPE", "EUC_2D"), ("CAPACITY",
                                                          capacity))
        ]))
        f.write("\n")
        f.write("NODE_COORD_SECTION\n")
        f.write("\n".join([
            "{}\t{}\t{}".format(i + 1, int(x / grid_size * 100000 + 0.5),
                                int(y / grid_size * 100000 +
                                    0.5))  # VRPlib does not take floats
            #"{}\t{}\t{}".format(i + 1, x, y)
        ...
        f.write("\n".join(
            ["{}\t{}".format(i + 1, d) for i, d in enumerate([0] + demand)]))
        f.write("\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("method",
                        help="Name of the method to evaluate, 'lkh' only")
    ...
    for dataset_path in opts.datasets:
        ...
        if opts.o is None:
            results_dir = os.path.join(opts.results_dir, "vrp",
                                       dataset_basename)
