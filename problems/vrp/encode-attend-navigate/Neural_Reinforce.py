...
input_batch = dataset.test_batch(batch_size=128,
                                 max_length=50,
                                 dimension=2,
                                 seed=123)  # Generate some data
...
data_arg.add_argument('--max_length',
                      type=int,
                      default=50,
                      help='number of cities')  ##### #####
data_arg.add_argument('--dimension',
                      type=int,
                      default=2,
                      help='city dimension')
data_arg.add_argument('--greedy',
                      action='store_true',
                      help='Use greedy decoding')

# Network
net_arg = add_argument_group('Network')
net_arg.add_argument('--input_embed',
                     type=int,
                     default=128,
                     help='actor critic input embedding')
net_arg.add_argument('--num_neurons',
                     type=int,
                     default=512,
                     help='encoder inner layer neurons')
net_arg.add_argument('--num_stacks',
                     type=int,
                     default=3,
                     help='encoder num stacks')
net_arg.add_argument('--num_heads',
                     type=int,
                     default=16,
                     help='encoder num heads')
net_arg.add_argument('--query_dim',
                     type=int,
                     default=360,
                     help='decoder query space dimension')
net_arg.add_argument('--num_units',
                     type=int,
                     default=256,
                     help='decoder and critic attention product space')
net_arg.add_argument('--num_neurons_critic',
                     type=int,
                     default=256,
                     help='critic n-1 layer')
...
train_arg.add_argument('--init_B',
                       type=float,
                       default=7.,
                       help='critic init baseline')
train_arg.add_argument('--lr_start',
                       type=float,
                       default=0.001,
                       help='actor learning rate')
train_arg.add_argument('--lr_decay_step',
                       type=int,
                       default=5000,
                       help='lr1 decay step')
train_arg.add_argument('--lr_decay_rate',
                       type=float,
                       default=0.96,
                       help='lr1 decay rate')
train_arg.add_argument('--temperature',
                       type=float,
                       default=1.0,
                       help='pointer initial temperature')
train_arg.add_argument('--C',
                       type=float,
                       default=10.0,
                       help='pointer tanh clipping')
train_arg.add_argument('--is_training',
                       type=str2bool,
                       default=True,
                       help='switch to inference mode when model is trained')
train_arg.add_argument('--n_test',
                       nargs='*',
                       type=int,
...
config, _ = get_config()
print(vars(config))
dir_ = str(config.dimension) + 'D_' + 'TSP' + str(
    config.max_length) + '_b' + str(config.batch_size) + '_e' + str(
        config.input_embed) + '_n' + str(config.num_neurons) + '_s' + str(
            config.num_stacks) + '_h' + str(config.num_heads) + '_q' + str(
                config.query_dim) + '_u' + str(config.num_units) + '_c' + str(
                    config.num_neurons_critic) + '_lr' + str(
                        config.lr_start) + '_d' + str(
                            config.lr_decay_step) + '_' + str(
                                config.lr_decay_rate) + '_T' + str(
                                    config.temperature) + '_steps' + str(
                                        config.nb_steps) + '_i' + str(
                                            config.init_B)
print(dir_)
...
class Actor(object):
    ...
    def build_reward(
        ...
        ordered_x_ = self.ordered_input_[
            0]  # ordered x, y coordinates [seq length +1, batch_size]
        ordered_y_ = self.ordered_input_[
            1]  # ordered y coordinates [seq length +1, batch_size]
        ...
        self.reward = tf.cast(self.distances,
                              tf.float32)  # define reward from tour length
    ...
    def build_optim(self):
        ...
        with tf.control_dependencies(
                update_ops):  # Update moving_mean and moving_variance for BN
...
with tf.Session() as sess:  # start session
    ...
    variables_names = [
        v.name for v in tf.trainable_variables() if 'Adam' not in v.name
    ]
...
with tf.Session() as sess:  # start session
    sess.run(tf.global_variables_initializer())  # run initialize op
    writer = tf.summary.FileWriter('summary/' + dir_,
    ...
    for epoch in range(config.nb_epochs):
        ...
        for i in tqdm(range(config.nb_steps),
                      mininterval=30):  # Forward pass & train step
            input_batch = dataset.train_batch(actor.batch_size,
                                              actor.max_length,
                                              actor.dimension)
            feed = {actor.input_: input_batch}  # get feed dict
            reward, predictions, summary, _, _ = sess.run([
                actor.reward, actor.predictions, actor.merged, actor.trn_op1,
                actor.trn_op2
            ],
                                                          feed_dict=feed)
        ...
        if epoch % 10 == 0:
            saver.save(sess, save_path +
                       "/actor{epoch}.ckpt")  # save the variables to disk

    if config.nb_epochs > 0:
        saver.save(sess,
                   save_path + "/actor.ckpt")  # save the variables to disk
...
def eval_dataset(ds, greedy, num_samples, save_path, max_length, seed=123):
    ...
    with tf.Session() as sess:  # start session
        ...
        saver.restore(sess, save_path +
                      "/actor.ckpt")  # Restore variables from disk.
        ...
        for i, coords in enumerate(tqdm(ds, mininterval=30)):
            ...
            feed = {actor.input_: input_batch}  # Get feed dict
            tour, reward = sess.run([actor.tour, actor.reward],
                                    feed_dict=feed)  # sample tours
            ...
            results.append(
                (calc_tsp_length(coords, best_permutation), best_permutation,
                 time.time() - start))

            opt_tour, opt_length = dataset.loop2opt(
                input_batch[0][best_permutation])
            ...
            opt_tour_ind = np.linalg.norm(opt_tour[:, None, :] -
                                          input_batch[0][None, :, :],
                                          axis=-1).argmin(-1)

            results_w2opt.append((calc_tsp_length(coords,
                                                  opt_tour_ind), opt_tour_ind,
                                  time.time() - start))

        predictions_length = np.asarray(
            predictions_length)  # average tour length
        predictions_length_w2opt = np.asarray(predictions_length_w2opt)
        print("Testing COMPLETED ! Mean length1:", np.mean(predictions_length),
              "Mean length2:", np.mean(predictions_length_w2opt))
...
def save_results(size, method, results, parallelism=1):
    ...
    out_file = os.path.join(results_dir,
                            "tsp{}_test_seed1234-{}.pkl".format(size, method))
...
for test_instance_size in config.n_test:
    ...
    results_greedy, results_greedy_2opt = eval_dataset(dataset_test, True, 1,
                                                       save_path,
                                                       test_instance_size)
    save_results(test_instance_size, f'deudon{train_size}greedy',
                 results_greedy)
    save_results(test_instance_size, f'deudon{train_size}greedy2opt',
                 results_greedy_2opt)
    results_sampling, results_sampling_2opt = eval_dataset(
        dataset_test, False, 1280, save_path, test_instance_size)
    save_results(test_instance_size, f'deudon{train_size}sampling',
                 results_sampling)
    save_results(test_instance_size, f'deudon{train_size}sampling2opt',
                 results_sampling_2opt)
