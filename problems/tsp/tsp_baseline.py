...
def solve_gurobi(directory,
    ...
    try:
        ...
        if os.path.isfile(problem_filename) and not disable_cache:
            ...
            cost, tour = solve_euclidian_tsp_gurobi(loc,
                                                    threads=1,
                                                    timeout=timeout,
                                                    gap=gap)
    ...
    for i in range(n):
        ...
        if method == 'random':
            ...
            if i == 0:
                ...
                a = feas_ind[D[np.ix_(
                    feas,
                    ~feas)].min(1).argmin()]  # node nearest to any in tour
            ...
            if i == 0:
                a = D.max(1).argmax(
                )  # Node with farthest distance to any other node
            else:
        ...
        if len(tour) == 0:
            ...
            ind_insert = np.argmin(
                _calc_insert_cost(D, tour, np.roll(tour, -1), a))
...
def calc_batch_pdist(dataset):
    ...
    return torch.matmul(diff[:, :, :, None, :],
                        diff[:, :, :, :, None]).squeeze(-1).squeeze(-1).sqrt()
...
def nearest_neighbour(dataset, start='first'):
    ...
    if not isinstance(start, torch.Tensor):
        if start == 'random':
            start = dataset.new().long().new(batch_size).zero_().random_(
                0, graph_size)
        elif start == 'first':
    ...
    dist_to_startnode = torch.gather(
        dist, 2,
        current.view(-1, 1, 1).expand(batch_size, graph_size, 1)).squeeze(2)
    tour = [current]

    for i in range(graph_size - 1):
        # Mark out current node as option
        dist.scatter_(2,
                      current.view(-1, 1, 1).expand(batch_size, graph_size, 1),
                      np.inf)
        nn_dist = torch.gather(
            dist, 1,
            current.view(-1, 1, 1).expand(batch_size, 1,
                                          graph_size)).squeeze(1)

        min_nn_dist, current = nn_dist.min(1)
        total_dist += min_nn_dist
        tour.append(current)

    total_dist += torch.gather(dist_to_startnode, 1,
                               current.view(-1, 1)).squeeze(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "method",
        help=
        "Name of the method to evaluate, 'nn', 'gurobi' or '(nearest|random|farthest)_insertion'"
    )
    ...
    parser.add_argument("--cpus",
                        type=int,
                        help="Number of CPUs to use, defaults to all cores")
    parser.add_argument('--no_cuda',
                        action='store_true',
                        help='Disable CUDA (only for Tsiligirides)')
    parser.add_argument('--disable_cache',
                        action='store_true',
                        help='Disable caching')
    parser.add_argument('--max_calc_batch_size',
                        type=int,
                        default=1000,
                        help='Size for subbatches')
    parser.add_argument('--progress_bar_mininterval',
                        type=float,
                        default=0.1,
    ...
    parser.add_argument('--offset',
                        type=int,
                        help="Offset where to start processing")
    ...
    assert opts.o is None or len(
        opts.datasets
    ) == 1, "Cannot specify result filename with more than one dataset"

    for dataset_path in opts.datasets:

        assert os.path.isfile(
            check_extension(dataset_path)), "File does not exist!"

        dataset_basename, ext = os.path.splitext(
            os.path.split(dataset_path)[-1])

        if opts.o is None:
            results_dir = os.path.join(opts.results_dir, "tsp",
                                       dataset_basename)
            os.makedirs(results_dir, exist_ok=True)

            out_file = os.path.join(
                results_dir, "{}{}{}-{}{}".format(
                    dataset_basename, "offs{}".format(opts.offset)
                    if opts.offset is not None else "",
        ...
        if method == "nn":
            ...
            results, parallelism = solve_all_nn(dataset_path, eval_batch_size,
                                                opts.no_cuda, opts.n,
                                                opts.progress_bar_mininterval)
        elif method in ("gurobi", "gurobigap", "gurobit", "concorde",
                        "lkh") or method[-9:] == 'insertion':
            ...
            if method == "concorde":
                ...
                executable = os.path.abspath(
                    os.path.join('problems', 'tsp', 'concorde', 'concorde',
                                 'TSP', 'concorde'))

                def run_func(args):
                    return solve_concorde_log(executable,
                                              *args,
                ...
                def run_func(args):
                    return solve_lkh_log(executable,
                                         *args,
                                         runs=runs,
                                         disable_cache=opts.disable_cache)
