    ...
    with open(filename, 'w') as f:
        ...
        f.write("\n".join([
            "{}\t{}\t{}".format(i + 1, int(x * 10000000 + 0.5),
                                int(y * 10000000 +
                                    0.5))  # oplib does not take floats
            #"{}\t{}\t{}".format(i + 1, x, y)
        ...
        f.write("\n".join(
            ["{}\t{}".format(i + 1, d) for i, d in enumerate([0] + prize)]))
        f.write("\n")
...
if __name__ == "__main__":
    executable = os.path.abspath(
        os.path.join('problems', 'op', 'compass', 'compass'))

    parser = argparse.ArgumentParser()
    parser.add_argument(
        "method",
        help="Name of the method to evaluate, 'compass', 'opga' or 'tsili'")
    ...
    for dataset_path in opts.datasets:
        ...
        if opts.o is None:
            results_dir = os.path.join(opts.results_dir, "op",
                                       dataset_basename)
        ...
        if method == "tsili" or method == "tsiligreedy":
            ...
            results, parallelism = run_all_tsiligirides(
                dataset_path, sample, num_samples, eval_batch_size,
                opts.max_calc_batch_size, opts.no_cuda, opts.n,
                opts.progress_bar_mininterval)
        elif method in ("compass", "opga", "gurobi", "gurobigap", "gurobit",
                        "ortools"):
            ...
            if method[:6] == "gurobi":
                ...
                def run_func(args):
                    return solve_compass_log(executable,
                                             *args,
                                             disable_cache=opts.disable_cache)
            elif method == "opga":
