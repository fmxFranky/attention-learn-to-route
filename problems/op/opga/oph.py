    ...
    while (found == True and len(s) > 0):
        ...
        for j in range(len(s)):
            for k in range(len(path) - 1):
                added_length = (
                    distance(path[k], s[j]) + distance(path[k + 1], s[j]) -
                    distance(path[k], path[k + 1]))  # optimize later
                if (length + added_length < tmax
                        and s[j][2] > max_added_reward):
...
def init_replacement(s1, start_point, end_point, tmax):
    ...
    return [
        x[1]
        for x in sorted([(sum([y[2] for y in z]), z) for z in paths])[::-1]
    ]
...
def initialize(s, start_point, end_point, tmax):
    ...
    dsub = sorted([(distance(x, start_point) + distance(x, end_point), x)
                   for x in s])[::-1]  #optimize later
    ls = dsub[:L]
    rest = dsub[L:]
    paths = []
    for i in range(L):
        path = [start_point, ls[i][1], end_point]
        length = ls[i][0]
        assert (length == distance(path[0], path[1]) +
        ...
        while (found == True and len(arest) > 0):
            ...
            for j in range(len(arest)):
                for k in range(len(path) - 1):
                    ...
                    if (length + added_length < tmax
                            and (added_length < min_added or min_added < 0)):
