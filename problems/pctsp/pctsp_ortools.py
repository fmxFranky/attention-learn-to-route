...
class DataProblem():
    ...
    def __init__(self, depot, loc, prize, penalty, min_prize):
        ...
        self._locations = [(float_to_scaled_int(l[0]),
                            float_to_scaled_int(l[1])) for l in [depot] + loc]
...
def euclidian_distance(position_1, position_2):
    ...
    return int(
        math.sqrt((position_1[0] - position_2[0])**2 +
                  (position_1[1] - position_2[1])**2) + 0.5)
...
def add_min_prize_constraints(routing, data, prize_evaluator, min_prize):
    ...
    for vehicle in xrange(data.num_vehicles):  # only single vehicle
        capacity_dimension.CumulVar(routing.End(vehicle)).RemoveInterval(
            0, min_prize)
...
def solve_pctsp_ortools(depot,
    ...
    nodes = [
        routing.AddDisjunction([int(c + 1)], p)
        for c, p in enumerate(data.penalties)
    ]
