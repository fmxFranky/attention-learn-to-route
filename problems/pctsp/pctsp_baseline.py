...
def get_pctsp_executable():
    ...
    if not os.path.isfile(execfile):
        print("Compiling...")
        check_call(
            ["g++", "-g", "-Wall", sourcefile, "-std=c++11", "-o", execfile])
        print("Done!")
    assert os.path.isfile(
        execfile), "{} does not exist! Compilation failed?".format(execfile)
    return os.path.abspath(execfile)


def solve_pctsp_log(executable,
                    directory,
                    name,
                    depot,
                    loc,
    ...
    problem_filename = os.path.join(directory,
                                    "{}.pctsp{}.pctsp".format(name, runs))
    output_filename = os.path.join(directory,
                                   "{}.pctsp{}.pkl".format(name, runs))
    ...
    try:
        ...
        return calc_pctsp_cost(depot, loc, penalty, deterministic_prize,
                               tour), tour.tolist(), duration
...
def solve_ortools(directory,
    ...
    try:
        ...
        if os.path.isfile(problem_filename) and not disable_cache:
            ...
            # 0 = start, 1 = end so add depot twice
            start = time.time()
            objval, tour = solve_pctsp_ortools(
                depot,
                loc,
                deterministic_prize,
                penalty,
...
def calc_pctsp_cost(depot, loc, penalty, prize, tour):
    ...
    return calc_pctsp_length(
        depot, loc, tour) + np.sum(penalty) - calc_pctsp_total(penalty, tour)


def write_pctsp(filename, depot, loc, penalty, prize, name="problem"):
    coord = [depot] + loc
    return write_pctsp_dist(filename, distance_matrix(coord, coord), penalty,
                            prize)


def float_to_scaled_int_str(
        v):  # Program only accepts ints so scale everything by 10^7
...
def write_pctsp_dist(filename, dist, penalty, prize):
    ...
    with open(filename, 'w') as f:
        f.write("\n".join([
            "",
            " ".join([float_to_scaled_int_str(p)
                      for p in [0] + list(prize)]), "", "",
            " ".join([float_to_scaled_int_str(p)
                      for p in [0] + list(penalty)]), "", "",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "method",
        help=
        "Name of the method to evaluate, 'pctsp', 'salesman' or 'stochpctsp(first|half|all)'"
    )
    ...
    for dataset_path in opts.datasets:
        ...
        if opts.o is None:
            results_dir = os.path.join(opts.results_dir, "pctsp",
                                       dataset_basename)
        ...
        if method in ("pctsp", "salesman", "gurobi", "gurobigap", "gurobit",
                      "ortools") or method[:10] == "stochpctsp":
