...
class PCTSP(object):
    ...
    def _get_costs(dataset, pi, stochastic=False):
        ...
        prize = dataset['stochastic_prize'] if stochastic else dataset[
            'deterministic_prize']
        prize_with_depot = torch.cat((torch.zeros_like(prize[:, :1]), prize),
                                     1)
        p = prize_with_depot.gather(1, pi)

        # Either prize constraint should be satisfied or all prizes should be visited
        assert ((p.sum(-1) >= 1 - 1e-5) |
                (sorted_pi.size(-1) -
                 (sorted_pi == 0).int().sum(-1) == dataset['loc'].size(-2))
                ).all(), "Total prize does not satisfy min total prize"
        penalty_with_depot = torch.cat(
            (torch.zeros_like(dataset['penalty'][:, :1]), dataset['penalty']),
            1)
        ...
        length = (
            (d[:, 1:] - d[:, :-1]).norm(p=2, dim=-1).sum(
                1)  # Prevent error if len 1 seq
    ...
    def __init__(self,
        ...
        if filename is not None:
            ...
            with open(filename, 'rb') as f:
                ...
                self.data = [{
                    'depot':
                    torch.FloatTensor(depot),
                    'loc':
                    torch.FloatTensor(loc),
                    'penalty':
                    torch.FloatTensor(penalty),
                    'deterministic_prize':
                    torch.FloatTensor(deterministic_prize),
                    'stochastic_prize':
                    torch.tensor(stochastic_prize)
                } for depot, loc, penalty, deterministic_prize,
                             stochastic_prize in (data[offset:offset +
                                                       num_samples])]
        else:
