...
from train import get_inner_model, rollout
...
class WarmupBaseline(Baseline):
    ...
    def epoch_callback(self, model, epoch):
        ...
        if epoch < self.n_epochs and torch.distributed.get_rank() == 0:
...
class CriticBaseline(Baseline):
    ...
    def eval(self, x, c):
        ...
        return v.detach(), F.mse_loss(v, c.detach()[:, None])
...
class RolloutBaseline(Baseline):
    ...
    def wrap_dataset(self, dataset):
        ...
        return BaselineDataset(
            dataset,
            rollout(self.model, dataset, self.opts).view(-1, 1).cpu())

    def unwrap_batch(self, batch):
        return batch['data'], batch['baseline'].view(
            -1)  # Flatten result to undo wrapping as 2D
