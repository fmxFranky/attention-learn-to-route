...
class AttentionModel(nn.Module):
    ...
    def propose_expansions(self,
        ...
        flat_parent = torch.arange(flat_action.size(-1),
                                   out=flat_action.new()) // ind_topk.size(-1)
        ...
        return flat_parent[feas_ind], flat_action[feas_ind], flat_score[
            feas_ind]
    ...
    def _inner(self, input, sep, embeddings):
        ...
        while not (self.shrink_size is None and state.all_finished()):
            ...
            if self.shrink_size is not None:
                ...
                if 16 <= len(
                        unfinished) <= state.ids.size(0) - self.shrink_size:
                    # Filter states
    ...
    def _precompute(self, embeddings, num_steps=1):
        ...
        fixed_attention_node_data = (self._make_heads(glimpse_key_fixed,
                                                      num_steps),
                                     self._make_heads(glimpse_val_fixed,
                                                      num_steps),
                                     logit_key_fixed.contiguous())
        return AttentionModelFixed(embeddings, fixed_context,
    ...
    def _get_log_p_topk(self, fixed, state, k=None, normalize=True):
        ...
        return (log_p,
                torch.arange(log_p.size(-1),
                             device=log_p.device,
                             dtype=torch.int64).repeat(log_p.size(0),
                                                       1)[:, None, :])
    ...
    def _one_to_many_logits(self, query, glimpse_K, glimpse_V, logit_K, mask,
        ...
        glimpse_Q = query.view(batch_size, num_steps, self.n_heads, 1,
                               key_size).permute(2, 0, 1, 3, 4)
        ...
        glimpse = self.project_out(
            heads.permute(1, 2, 3, 0,
                          4).contiguous().view(-1, num_steps, 1,
                                               self.n_heads * val_size))
        ...
        logits = torch.matmul(final_Q, logit_K.transpose(
            -2, -1)).squeeze(-2) / math.sqrt(final_Q.size(-1))
    ...
    def _make_heads(self, v, num_steps=None):
        ...
        return (v.contiguous(
        ).view(v.size(0), v.size(1), v.size(2), self.n_heads, -1).expand(
            v.size(0),
            v.size(1) if num_steps is None else num_steps, v.size(2),
            self.n_heads, -1).permute(
                3, 0, 1, 2,
                4)  # (n_heads, batch_size, num_steps, graph_size, head_dim)
                )
