...
class Attention(nn.Module):
    ...
    def forward(self, query, ref):
        ...
        v_view = self.v.unsqueeze(0).expand(expanded_q.size(0),
                                            len(self.v)).unsqueeze(1)
...
class Decoder(nn.Module):
    ...
    def recurrence(self, x, h_in, prev_mask, prev_idxs, step, context):
        ...
        logit_mask = self.update_mask(
            prev_mask, prev_idxs) if prev_idxs is not None else prev_mask

        logits, h_out = self.calc_logits(x, h_in, logit_mask, context,
                                         self.mask_glimpses, self.mask_logits)
    ...
    def forward(self,
        ...
        idxs = None
        mask = Variable(embedded_inputs.data.new().byte().new(
            embedded_inputs.size(1), embedded_inputs.size(0)).zero_(),
                        requires_grad=False)
        ...
        for i in steps:
            ...
            idxs = self.decode(
                probs, mask) if eval_tours is None else eval_tours[:, i]
            ...
            decoder_input = torch.gather(
                embedded_inputs, 0,
                idxs.contiguous().view(1, batch_size, 1).expand(
                    1, batch_size,
                    *embedded_inputs.size()[2:])).squeeze(0)
...
class CriticNetworkLSTM(nn.Module):
    ...
    def forward(self, inputs):
        ...
        encoder_hx = self.encoder.init_hx.unsqueeze(0).repeat(
            inputs.size(1), 1).unsqueeze(0)
        encoder_cx = self.encoder.init_cx.unsqueeze(0).repeat(
            inputs.size(1), 1).unsqueeze(0)

        # encoder forward pass
        enc_outputs, (enc_h_t,
                      enc_c_t) = self.encoder(inputs, (encoder_hx, encoder_cx))
        ...
        for i in range(self.n_process_block_iters):
            ...
            process_block_state = torch.bmm(
                ref,
                self.sm(logits).unsqueeze(2)).squeeze(2)
...
class PointerNetwork(nn.Module):
    def __init__(self,
        ...
        self.embedding = nn.Parameter(
            torch.FloatTensor(self.input_dim, embedding_dim))
    ...
    def forward(self, inputs, eval_tours=None, return_pi=False):
        ...
        embedded_inputs = torch.mm(
            ...
            self.embedding).view(graph_size, batch_size, -1)
    ...
    def _inner(self, inputs, eval_tours=None):
        ...
        encoder_hx = encoder_cx = Variable(torch.zeros(1,
                                                       inputs.size(1),
        ...
        decoder_input = self.decoder_in_0.unsqueeze(0).repeat(
            inputs.size(1), 1)

        (pointer_probs,
         input_idxs), dec_hidden_t = self.decoder(decoder_input, inputs,
                                                  dec_init_state, enc_h,
                                                  eval_tours)
